...
public abstract class BeanManagedViewAbstract extends BeanReportView {
	...
	// protected abstract void orderReverse(List<ObjetoCampoConsulta> listTemp); === Foi criado pea a IDE
	...
	public int totalRegistroConsulta() throws Exception {
		Query query = (Query) getController().obterQuery(" select count(entity) from " + getQueryConsulta());
		Number result = (Number) ((org.hibernate.Query) query).uniqueResult();
